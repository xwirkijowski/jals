...
export const LinkNotFound = ({linkId, context}: {linkId?: string, context: string}): React.ReactNode => {
	return (
		<motion.div variants={container} className={cx('flex-col gap-4 flex items-center max-w-lg w-full')}>
			...
			{context === 'dashboard' && (
				<motion.div variants={container} className={'flex gap-4'}>
					<Link href={`/dashboard`} passHref><Button variants={item} btnType={'primary'} effects={true}>Dashboard</Button></Link>
				</motion.div>
			)}
			{context === 'redirect' && (
				<motion.div variants={container} className={'flex gap-4'}>
